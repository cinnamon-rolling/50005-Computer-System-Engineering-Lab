...
public class DesImageSolution {
    public static void main(String[] args) throws Exception{
        ...
        BufferedImage img = ImageIO.read(new File("src/SUTD.bmp"));
...
//you need to try both ECB and CBC mode, use PKCS5Padding padding method
        ...
        for(int idx = 0; idx < image_width; idx++) {
        // for(int idx = (image_width-1); idx >= 0; idx--) {
...
// TODO: encrypt each column or row bytes
            byte[] encrypted_each_width_pixel = desCipher.doFinal(each_width_pixel);
            System.out.println("width of pixel " + each_width_pixel.length);
            System.out.println("en width of pixel " + encrypted_each_width_pixel.length);
...
// TODO: convert the encrypted byte[] back into int[] and write to outImage (use setRGB)
         ...
         for (int idy = 0; idy < image_length; idy++){
             System.arraycopy(encrypted_each_width_pixel, idy*4, encrypted_pixel, 0 ,4);
...
//write outImage into file
        ImageIO.write(outImage, "BMP",new File("src/oppCBCEncryptSUTD.bmp"));
    }
}