...
int checkSafe(int customerIndex, int *request)
{
	...
	bool *finish = mallocIntVector(numberOfCustomers);
	for (int i = 0; i < numberOfCustomers; i++)
	{
		finish[i] = false;
	}
...
int requestResources(int customerIndex, int *request)
{
	...
	{
		if (need[customerIndex][x] < request[x])
		{
			return 0;
		}
		// TODO: judge if request larger than available
		else if (available[x] < request[x])
		{
			return 0;
		}
	}
	...
	if (checkSafe(customerIndex, request))
	{
		for (int x = 0; x < numberOfResources; x++)
		{
			...
			allocation[customerIndex][x] += request[x];
			need[customerIndex][x] -= request[x];
		}
		return 1;
	}
	else
...
void releaseResources(int customerIndex, int *release)
{
	...
	for (int x = 0; x < numberOfResources; x++)
	{
		available[x] += release[x];
		allocation[customerIndex][x] -= release[x];
		need[customerIndex][x] += release[x];
	}
}
